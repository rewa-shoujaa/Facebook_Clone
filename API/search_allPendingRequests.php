...
//echo($search);
$response = array();
$sql = 'Select * from users_tbl inner join connections_tbl on users_tbl.user_id = connections_tbl.cnx_targetUserID where connections_tbl.cnx_sourceUserID="' . $userID . '" and user_id IN (Select cnx_targetUserID from connections_tbl where cnx_sourceUserID="' . $userID . '" and cnx_isPending="1") AND user_firstName LIKE "' . $search . '" OR user_lastName LIKE "' . $search . '"';
