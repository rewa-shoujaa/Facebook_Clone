...
<head>
  ...
  <body>
      ...
      $(function() {
        ...
        async function fetch_Network_API(){
          try{
				result = await $.ajax({
					type: "POST",
					url: "http://localhost/Facebook_Project-RewaShoujaa/API/get_pendingRequests.php",
					data: {
            userID:User_ID
                        },
        ...
        for(var user in results) {
            newUser=newUser+'<div id="user_'+results[user]["connection_id"]+'">'+
            '<div class="row" style="margin-bottom:10px;">'+
                  ...
                  '<div class="col-md-2 col-sm-2">'+
                    '<button class="btn btn-primary pull-right" id="Cancel_'+results[user]["connection_id"]+'" onclick="Cancel(this.id)">Cancel</button>'+
                  '</div>'+
                '</div>'+
                '</div>';
                }
    ...
    async function getSearchResults_API(SearchValue){
      //var Pending_Users;
          try{
				result = await $.ajax({
                   type: "POST",
                   url: "http://localhost/Facebook_Project-RewaShoujaa/API/search_allPendingRequests.php",
                     data: {
                        userID: User_ID,
    ...
    $('#SearchForm').on('submit',  function(e) {
        ...
        for(var user in results) {
            ...
            '<div class="row" style="margin:8px;">'+
                  ...
                  '</div>'+
                  '<div class="col-md-2 col-sm-2">'+
                    '<button class="btn btn-primary pull-right" id="add_'+results[user]["connection_id"]+'" onclick="Cancel(this.id)">Cancel</button>'+
                  '</div>'+
                '</div>'+
                '</div>';
                }
    ...
    async function Add_User_API(id){
      ...
      //var Pending_Users;
          try{
				result = await $.ajax({
					type: "POST",
					url: "http://localhost/Facebook_Project-RewaShoujaa/API/cancel_pendingRequest.php",
					data: {
            ConnectionID:id,
                        }
				})
    ...
    async function Cancel_Request_API(id){
      //var Pending_Users;
          try{
				result = await $.ajax({
					type: "POST",
      ...
      //var id=$(this).attr("id");
      console.log(id);
      var cnxID=id.split("_");
      cnxID=cnxID[1];
      console.log(cnxID);
      Cancel_Request_API(cnxID).then(results => {
        //console.log(results);
        $("#user_"+cnxID).hide();
      }).catch(error => {
				console.log(error.message);
