  ...
  <body>
    ...
    <script>
      ...
      $(function() {
        ...
        async function fetch_Friends_API(){
          try{
				result = await $.ajax({
					type: "POST",
					url: "http://localhost/Facebook_Project-RewaShoujaa/API/get_allFriends.php",
					data: {
            userID:User_ID
                        },
		...
		function getAllUsers(){
			fetch_Friends_API().then(results => {
        //console.log(results);
        var newUser="";
        for(var user in results) {
            ...
            '<div class="row" style="margin-bottom:10px;">'+
                  ...
                  '<div class="col-md-2 col-sm-2">'+
                    '<button class="btn btn-primary pull-right" id="add_'+results[user]["id"]+'" onclick="UnfriendUser(this.id)">unfriend</button>'+
                  '</div>'+
                  '<div class="col-md-2 col-sm-2">'+
                    '<button class="btn btn-danger pull-right" id="block_'+results[user]["id"]+'" onclick="BlockUser(this.id)">Block</button>'+
                  '</div>'+
                '</div>'+
                '</div>';
                }
    ...
    async function getSearchResults_API(SearchValue){
      //var Pending_Users;
          try{
				result = await $.ajax({
                   type: "POST",
                   url: "http://localhost/Facebook_Project-RewaShoujaa/API/search_allFriends.php",
                     data: {
                        userID: User_ID,
    ...
    $('#SearchForm').on('submit',  function(e) {
        ...
        for(var user in results) {
            ...
            '<div class="row" style="margin:8px;">'+
                  ...
                  '</div>'+
                  '<div class="col-md-2 col-sm-2">'+
                    '<button class="btn btn-primary pull-right" id="add_'+results[user]["id"]+'" onclick="AddUser(this.id)">Add</button>'+
                  '</div>'+
                  '<div class="col-md-2 col-sm-2">'+
                    '<button class="btn btn-danger pull-right" id="block_'+results[user]["id"]+'" onclick="BlockUser(this.id)">Block</button>'+
                  '</div>'+
    ...
    async function Unfiend_User_API(id){
      var user_name = document.getElementById("userName_hidden").value;
      console.log("user_name");
      ...
      //var Pending_Users;
          try{
				result = await $.ajax({
					type: "POST",
					url: "http://localhost/Facebook_Project-RewaShoujaa/API/unfriend_user.php",
					data: {
            AddedUserID:id,
            userID:User_ID,
                        }
      ...
      UnfriendUser =function(id){
      //var id=$(this).attr("id");
      console.log(id);
      var id_pending=id.split("_");
      id_pending=id_pending[1];
      console.log(id_pending);
      Unfiend_User_API(id_pending).then(results => {
        //console.log(results);
    ...
    async function Block_User_API(id){
      //var Pending_Users;
          try{
				result = await $.ajax({
					type: "POST",
					url: "http://localhost/Facebook_Project-RewaShoujaa/API/block_friend.php",
					data: {
            AddedUserID:id,
            userID:User_ID
                        }
      ...
      id_pending=id_pending[1];
      console.log(id_pending);
      Block_User_API(id_pending).then(results => {
        //console.log(results);
        $("#user_"+id_pending).hide();
      }).catch(error => {
				console.log(error.message);
